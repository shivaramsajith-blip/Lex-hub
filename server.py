...
app.secret_key = os.environ.get('SECRET_KEY', 'lexhub-secret-2026')
...
def init_db():
    ...
    existing_pw = conn.execute("SELECT value FROM settings WHERE key='password_hash'").fetchone()
    if not existing_pw:
        default_hash = hashlib.sha256('kmq2026'.encode()).hexdigest()
        conn.execute("INSERT OR REPLACE INTO settings VALUES ('password_hash',?)", (default_hash,))
    conn.commit()
    conn.close()
...
def login():
    ...
    conn = get_db()
    stored = conn.execute("SELECT value FROM settings WHERE key='password_hash'").fetchone()
    conn.close()
    if stored and stored['value'] == pw_hash:
...
def change_password():
    ...
    if len(new_pw) < 4: return jsonify({'error': 'Password too short'}), 400
    pw_hash = hashlib.sha256(new_pw.encode()).hexdigest()
    conn = get_db()
    conn.execute("INSERT OR REPLACE INTO settings VALUES ('password_hash',?)", (pw_hash,))
...
def get_settings():
    conn = get_db()
    rows = conn.execute("SELECT key,value FROM settings WHERE key != 'password_hash'").fetchall()
    conn.close()
...
def export_data():
    ...
    settings = {r['key']:r['value'] for r in conn.execute("SELECT key,value FROM settings WHERE key != 'password_hash'").fetchall()}
